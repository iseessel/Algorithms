...
class Board
  attr_accessor :rows, :best_move, :turn

  def initialize(turn, rows = self.class.blank_grid)
    @rows = rows
    @turn = turn
  end
  ...
  def possible_moves # works
    moves = []

    (0..2).each do |row|
      (0..2).each do |col|
        pos = [row, col]
        moves << pos if self.empty?(pos)
      end
    end
  ...
  def create_new_board(move) # works

    deep_duped_board = Marshal.load(Marshal.dump(self))
    deep_duped_board[move] = self.turn
    new_turn = (self.turn == :x ? :o : :x)
    deep_duped_board.turn = new_turn

    deep_duped_board
  end

  def value
    return 1 if self.winner == :x
    return 0 if self.winner.nil?
    return -1 if self.winner == :o
  end
